...
def download_data():
    ...
    blobs = bucket.list_blobs(prefix="labeled_data/")

    # Download annotations
    for blob in blobs:
        print("Annotation file:", blob.name)

        if not blob.name.endswith("labeled_data/"):
    ...
    for annotation_file in annotation_files:
        ...
        with open(annotation_file, "r") as read_file:
            ...
            if (success):
                ...
                label = annotation_json["label"]["date"] + "_" + \
                    annotation_json["label"]["time"] + "_" + \
                    annotation_json["label"]["subject"]

                local_file_path = os.path.join(dataset_folder, label)
                blob.download_to_filename(local_file_path)

                source_path = os.path.join(
                    dataset_prep_folder, annotation_file)
                destination_path = os.path.join(dataset_folder, label)
                # Copy the file
